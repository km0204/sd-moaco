...
The Python packages are declared in `requirements.txt`. No proprietary solver,
commercial software, or GPU is required.
...
```bash
git clone https://github.com/km0204/simulation-driven-moaco.git
cd simulation-driven-moaco
```
...
All commands below should be run from the repository root while the virtual
...
On successful completion, the command prints the number of jobs and requests,
the number of completed iterations, the final archive size, objective values,
and the result of the executable validation checks.
...
Both execution modes use deterministic reference seeds defined in `config.py`.
The paper-settings mode can take substantially longer than quick mode because
every complete candidate policy is evaluated under all five disturbance
scenarios.
...
The directory is created automatically. Running another mode writes the current
run's summaries to the same filenames, replacing the previous CSV contents.
